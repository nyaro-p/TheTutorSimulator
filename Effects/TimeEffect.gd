...
func animate_timer(time):
	...
	tween.tween_property(bounds_mesh, "scale", Vector2(1.0, 1.0), 1.0)
	...
	tween.parallel().tween_property(growing_mesh, "modulate", Color(1, 0, 0), time).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT)
